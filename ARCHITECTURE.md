...
| --------------------- | ----------------------------------------------------- | -------------------------------------- | ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Date Range / Year** | Named preset buttons + custom range picker            | `Button` (×4) + `Popover` + `Calendar` | `CrashDate`            | Four preset buttons: **YTD** (Jan 1 → max available), **90 Days** (max−90d → max), **Last Year** (previous full calendar year), **3 Years** (max−36mo → max). All end-anchored to `dataBounds.maxDate` so they never exceed available data. Custom range picker below for arbitrary ranges. Active button stays highlighted on URL reload via named `DatePreset` type in filter state. |
...
map.addLayer({
  ...
  paint: {
    ...
    'circle-stroke-color': [
      ...
      ['get', 'mode'],
      'Bicyclist',
      '#1565C0', // Blue stroke for bicyclists
      'Pedestrian',
      '#4A148C', // Purple stroke for pedestrians
      '#666666',
    ],
    'circle-stroke-width': [
      'interpolate',
      ...
      ['zoom'],
      4,
      0.5, // Thin stroke when zoomed out
      10,
      1.5, // Medium stroke at city level
      16,
      2.5, // Thicker stroke at street level
    ],
  },
